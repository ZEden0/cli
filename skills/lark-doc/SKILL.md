...
| [`+cover-get` / `+cover-update` / `+cover-delete`](references/lark-doc-cover.md) | Get, update, or delete a Docx cover. Use `+media-upload --parent-type docx_image --parent-node <document_id> --doc-id <document_id>` to prepare local image tokens; do not reuse body image block tokens. |
